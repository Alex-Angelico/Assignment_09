...
while True:
    ...
    if strChoice == 'l':
        ...
        while True:
            ...
            strCDmenuChoice = IO.ScreenIO.menu_CD_choice()
            if strCDmenuChoice == 'x':
                break
            if strCDmenuChoice == 'a':
                tplTrackinInfo = IO.ScreenIO.get_track_info()
                PC.DataProcessor.add_track(tplTrackinInfo, cd)
            if strCDmenuChoice == 'd':
                IO.ScreenIO.show_tracks(cd)
            if strCDmenuChoice == 'r':
                track_idDel = int(input('Enter track number to remove: '))
                PC.DataProcessor.rmv_track(track_idDel,cd)
                pass
    elif strChoice == 's':
        IO.ScreenIO.show_inventory(lstOfCDObjects)
